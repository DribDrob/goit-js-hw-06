...
function createBoxes(amount) {
 let boxElementsArr = [];
  ...
  if (refs.boxesEl.children.length !== 0){
    boxEl.style.width =  `${Number.parseInt(refs.boxesEl.lastElementChild.style.width)+10}px`;
    boxEl.style.height =  `${Number.parseInt(refs.boxesEl.lastElementChild.style.height)+10}px`;
  }
  boxElementsArr.push(boxEl);
}
refs.boxesEl.append(...boxElementsArr);
